...
static c_block *i_cblock_constructor(cdrom_paranoia_t *p){
...
c_block *new_c_block(cdrom_paranoia_t *p){
...
c_block *c_first(cdrom_paranoia_t *p){
  if(p->cache->head)
    return(p->cache->head->ptr);
  return(NULL);
}

c_block *c_last(cdrom_paranoia_t *p){
  if(p->cache->tail)
...
v_fragment *v_first(cdrom_paranoia_t *p){
  if(p->fragments->head){
...
v_fragment *v_last(cdrom_paranoia_t *p){
  if(p->fragments->tail)
...
void i_paranoia_firstlast(cdrom_paranoia_t *p){
  ...
  if(p->current_lastsector==-1)
    p->current_lastsector=xld_cdda_disc_lastsector_currentsession(d,p->cursor);
...
cdrom_paranoia_t *
paranoia_init_old(cdrom_drive_t *d)
{
	cdrom_paranoia_t *p=calloc(1,sizeof(cdrom_paranoia_t));
