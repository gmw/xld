...
static int time_drive(cdrom_drive *d, FILE *progress, FILE *log, int lba, int len, int initial_seek, short *buf){
  int i,x;
  int latency=0;
  double sum=0;
  double sumsq=0;
  ...
  for(i=0,sofar=0;sofar<len;i++){
    ...
    if((ret=cdda_read_timed(d,buf,lba+sofar,toread,&x))<=0){
...
static float retime_drive(cdrom_drive *d, FILE *progress, FILE *log, int lba, int readahead, float oldmean, short *buf){
  int sectors = 2000;
  int total;
  float newmean;
  ...
  total = time_drive(d,NULL,log,lba,sectors,1,buf);
...
int analyze_cache(cdrom_drive *d, FILE *progress, FILE *log, int speed, short *buf, cache_analysis_t *result_t){
     ...
     eyes.  Take readings ten minutes apart (45000 sectors) and at end of disk. */
  {
	...
	logC("\n");
      ...
      if((ret=cdda_read(d,buf,offset+current+1,1))<0){
	/* media error! grr!  retry elsewhere */
	if(ret==-404) return -1;
	...
	continue;
      ...
      sofar=time_drive(d,progress, log, offset, current, 1, buf);
     ...
     bisection */
  {
	...
	for(j=0;;j++){
	  ...
	  if(i>=15){
	    ...
	    while(sofar<current){
	      ret1 = cdda_read_timed(d,buf,offset+sofar,current-sofar,&x);
	      logC("slow_read=%d:%d:%d ",offset+sofar,ret1,x);
	      if(ret1<=0)break;
	      sofar+=ret1;
	    }
	  }else{
	    ret1 = cdda_read_timed(d,buf,offset+current-1,1,&x);
	  ...
	  ret2 = cdda_read_timed(d,buf,offset,1,&x);
  ...
  if(cachesize==hi){
	...
	result_t->cache_sector_size = -1;
    return 1;
  }else if(cachesize){
    reportC("\tApproximate random access cache size: %d sector(s)               \n",cachesize);
	result_t->cache_sector_size = cachesize;
	result_t->have_cache = YES;
  }else{
    reportC("\tDrive does not cache nonlinear access                            \n");
	result_t->cache_sector_size = 0;
	result_t->have_cache = NO;
    return 0;
  }
     ...
     differently if non-contiguous.... */
  {
	...
	ret1 = cdda_read_timed(d,buf,offset+seekoff,1,&x);
  ...
  */

  {
	...
	while(sofar<cachesize){
	  ret = cdda_read_timed(d,buf,offset+sofar,cachesize-sofar,&x);
	  if(ret<=0)goto error;
	...
	ret = cdda_read_timed(d,buf,offset+cachesize+readahead-1,1,&x);
	if(ret<=0)break;
	...
	if(x<MIN_SEEK_MS){
	  ...
	  mspersector=retime_drive(d, progress, log, offset, readahead, mspersector, buf);
	}
      }
  ...
  while(1){
	...
	usleep(usec);
      ...
      while(sofar>0){
	sofar--;
	ret = cdda_read_timed(d,buf,offset+sofar,1,&x);
	if(ret<=0)break;
    ...
    {
      float newms=retime_drive(d, progress, log, offset, readahead, mspersector, buf);
  ...
  while(1){
	...
	ret = cdda_read_timed(d,buf,offset+sofar,cachesize-sofar+1,&x);
  ...
  /* sample cache access for five realtime seconds. */
  {
    ...
    if(readsize<7){
	 ...
	 'initial_seek' on time_drve */
      ...
      while(1){
	int ret=time_drive(d, NULL, log, offset, readsize, 0, buf);
	if(ret==-404) return -1;
	if(ret>0)break;
	retry++;
	if(retry==10){
	...
	while(elapsed<5000){
	  sectors += readsize;
	  elapsed += time_drive(d, NULL, log, offset, readsize, 0, buf);
	...
	reportC("\tTesting that backseek flushes cache...");
	{
	  ...
	  while(elapsed<5000 && total<25){ /* don't kill the drive */
	    ...
	    while(1){
	      ...
	      ret=cdda_read(d, buf, seekpos, 1);
	      if(ret>0) ret=time_drive(d, NULL, log, offset+1, readsize, 1, buf);
	      if(ret>=0) ret=time_drive(d, NULL, log, offset, readsize, 1, buf);
	  ...
	  if(readms*2. < mspersector ||
	    ...
	    warn=1;
		result_t->backseek_flush_capable = 0;
	  }else{
	    reportC("\tBackseek flushes the cache as expected\n");
		result_t->backseek_flush_capable = 1;
	  }
	}
      }
    }
